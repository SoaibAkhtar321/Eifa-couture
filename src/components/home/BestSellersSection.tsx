            ...
            return (
              ...
              >
                <Link href={`/product/${product.slug}`} className="block">
                  <div
                    className={`grid grid-cols-1 md:grid-cols-2 gap-0 md:gap-0 items-stretch border-t border-beige ${
                      idx === products.length - 1 ? 'border-b' : ''
                    }`}
                  >
                    {/* Image Side */}
                    <div className={`img-zoom relative aspect-[4/3] md:aspect-auto overflow-hidden bg-beige ${isEven ? 'md:order-1' : 'md:order-2'}`}>
                      <img
                      ...
                      <span className="absolute top-6 left-6 md:top-8 md:left-8 font-heading text-7xl md:text-9xl text-white/20 font-bold leading-none select-none">
                        {String(idx + 1).padStart(2, '0')}
                      </span>
                    </div>

                    {/* Content Side */}
                    <div className={`flex flex-col justify-center p-8 md:p-12 lg:p-16 bg-white ${isEven ? 'md:order-2' : 'md:order-1'}`}>
                      <div>
                        ...
                        <h3 className="font-heading text-2xl md:text-3xl lg:text-4xl text-charcoal group-hover:text-maroon transition-colors duration-300 leading-tight">
                          {product.name}
                        </h3>

                        <p className="font-body text-charcoal/60 text-sm md:text-base mt-4 leading-relaxed max-w-md">
                          {product.shortDescription}
                        </p>

                        <div className="flex items-center gap-3 mt-6">
                          <span className="font-subheading text-2xl md:text-3xl text-charcoal font-semibold">
