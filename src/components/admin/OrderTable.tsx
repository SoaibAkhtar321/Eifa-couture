  ...
  const columns: DataTableColumn<OrderListRow>[] = [
    {
      ...
      render: (row) => (
        <div>
          <p className="font-medium text-charcoal">{row.orderNumber}</p>
          <p className="text-xs text-charcoal/50">{formatDate(row.placedAt, 'short')}</p>
        </div>
      ),
    },
    {
    ...
    { key: 'total', header: 'Total', render: (row) => formatPrice(row.total) },
    {
      key: 'payment',
      header: 'Payment',
      render: (row) => <span className="capitalize text-charcoal/70">{row.paymentStatus}</span>,
    },
    {
      key: 'status',
      header: 'Status',
      render: (row) => (
        <span
          className={`rounded-full px-2.5 py-1 text-xs font-medium capitalize ${STATUS_BADGE[row.status] ?? STATUS_BADGE.pending}`}
        >
